...
export const quickAddSchema = z
  ...
  .superRefine((values, context) => {
    if (requiredAmountActions.includes(values.action) && !values.amount)
      context.addIssue({
        code: "custom",
        path: ["amount"],
        message: "Amount is required",
      });
    if (requiredValueActions.includes(values.action) && !values.value)
      context.addIssue({
        code: "custom",
        path: ["value"],
        message: "Weight is required",
      });
    if (durationActions.includes(values.action) && !values.duration)
      context.addIssue({
        code: "custom",
        path: ["duration"],
    ...
    if (
      ["job-application", "application-update"].includes(values.action) &&
      !values.company
    )
      context.addIssue({
        code: "custom",
    ...
    if (values.action === "job-application" && !values.role)
      context.addIssue({
        code: "custom",
        path: ["role"],
