...
import {
  quickAddSchema,
  type QuickAddValues,
} from "@/features/quick-add/schemas/quick-add-schema";
...
const defaults = (action: QuickAddValues["action"]): QuickAddValues => ({
  action,
  date: today(),
  recurring: false,
  title: "",
...
function financeCategory(
  ...
  return (transactionCategories as readonly string[]).includes(value ?? "")
...
function eventType(value: string | undefined): CalendarEventType {
  const values: CalendarEventType[] = [
    "study",
    "workout",
    "interview",
    "bill",
  ...
  return values.includes(value as CalendarEventType)
...
export function QuickAddModal() {
  ...
  const form = useForm<QuickAddValues>({
    ...
    defaultValues: defaults(selectedAction ?? "note"),
  });
  useEffect(() => {
    if (selectedAction) form.reset(defaults(selectedAction));
  }, [form, selectedAction]);
  const actionConfig = quickAddActions.find(
  ...
  const onSubmit = (values: QuickAddValues) => {
    if (values.action === "expense" || values.action === "income") {
      addTransaction({
        description:
          values.notes ||
          (values.action === "expense"
            ? "Quick-added expense"
            : "Quick-added income"),
        amount: values.amount ?? 0,
        type: values.action,
        category: financeCategory(values.category, values.action === "income"),
        date: values.date,
      ...
      logWeight({
        date: values.date,
        weightKg: values.value ?? 0,
        note: values.notes,
      });
    } else if (values.action === "meal") {
      logMeal({
        date: values.date,
        mealName: values.title ?? "Meal",
        calories: values.calories ?? 0,
        proteinGrams: values.protein ?? 0,
        note: values.notes,
      });
    } else if (values.action === "workout") {
      logWorkout({
        date: values.date,
        workoutName: values.title ?? "Workout",
        durationMinutes: values.duration ?? 0,
        intensity: "moderate",
        note: values.notes,
      });
    } else if (values.action === "study-session") {
      addStudySession({
        category: learningCategory(values.category),
        topic: values.title ?? "Focused study",
        durationMinutes: values.duration ?? 0,
        studiedAt: `${values.date}T18:00:00.000Z`,
        focusScore: 4,
        notes: values.notes,
      });
      ...
      addApplication({
        ...
        priority: "standard",
        discoveredAt: values.date,
        appliedAt:
          applicationStage(values.stage) === "Applied"
            ? values.date
            : undefined,
        source: "Quick add",
        initialNote: values.notes,
      });
    } else if (values.action === "application-update") {
      const match = applications.find(
        (application) =>
          application.company.toLowerCase() === values.company?.toLowerCase(),
      );
      if (!match) {
        toast.error("Application not found", {
          description: `No application matches “${values.company}”. Add it first or update it from Career.`,
        });
        return;
      }
      ...
      const project =
        projects.find((candidate) => candidate.name === values.project) ??
        projects[0];
      if (!project) {
      ...
      addCalendarEvent({
        ...
        description: values.notes,
        type: eventType(values.category),
    ...
    toast.success(`${actionConfig?.label ?? "Entry"} saved`, {
      description: "Your session data has been updated.",
  ...
  return (
    ...
    >
      <DialogContent className="bottom-0 left-0 top-auto max-h-[94vh] max-w-none translate-x-0 translate-y-0 overflow-y-auto rounded-b-none rounded-t-3xl p-0 data-[state=open]:slide-in-from-bottom sm:left-1/2 sm:top-1/2 sm:max-h-[88vh] sm:max-w-2xl sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-2xl">
        <DialogHeader className="sticky top-0 z-10 border-b border-border bg-popover/95 px-5 py-5 pr-12 backdrop-blur-xl sm:px-6">
          {selectedAction ? (
            <div className="mb-2">
              <Button
                type="button"
          ...
          <DialogDescription>
            {actionConfig?.description ??
              "Capture an update anywhere in your personal operating system."}
          </DialogDescription>
        </DialogHeader>
        {!selectedAction ? (
          <div className="grid gap-2 p-5 sm:grid-cols-2 sm:p-6">
            {quickAddActions.map((action) => {
              ...
              return (
                <button
                  ...
                  className="group flex items-start gap-3 rounded-xl border border-border bg-background/35 p-3.5 text-left transition hover:-translate-y-0.5 hover:border-primary/25 hover:bg-primary/[0.055]"
                >
                  <span className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-secondary text-muted-foreground transition group-hover:bg-primary/10 group-hover:text-indigo-300">
            ...
            <div className="col-span-full mt-2 flex items-center justify-center gap-2 text-[11px] text-muted-foreground">
          ...
          >
            {["expense", "income"].includes(selectedAction) ? (
              <>
                <div>
            ...
            {selectedAction === "weight" ? (
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <Label htmlFor="qa-value">Weight (kg)</Label>
                  <Input
                    id="qa-value"
                    className="mt-2"
                    type="number"
            ...
            {["workout", "study-session"].includes(selectedAction) ? (
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <Label htmlFor="qa-duration">Duration (minutes)</Label>
                  <Input
                    id="qa-duration"
                    className="mt-2"
                    type="number"
                ...
                <div>
                  <Label htmlFor="qa-category">
                    {selectedAction === "workout" ? "Workout type" : "Topic"}
                  </Label>
                  <Input
                    id="qa-category"
                    className="mt-2"
                    placeholder={
                      selectedAction === "workout" ? "Strength" : "Python"
                    }
                    {...form.register("category")}
                  />
                </div>
            ...
            {["job-application", "application-update"].includes(
              selectedAction,
            ) ? (
              <>
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  {selectedAction === "job-application" ? (
                    <div>
            ...
            <div>
              ...
              <Textarea
                id="qa-notes"
                className="mt-2"
                placeholder="Add context that will be useful later…"
                {...form.register("notes")}
              />
            </div>
            <div className="sticky bottom-0 -mx-5 -mb-5 flex items-center justify-end gap-2 border-t border-border bg-popover/95 px-5 py-4 backdrop-blur-xl sm:-mx-6 sm:-mb-6 sm:px-6">
