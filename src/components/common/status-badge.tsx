...
function inferTone(status: string): Tone {
  ...
  if (
    ...
    return "success";
  if (
    ["blocked", "overdue", "rejected", "failed", "high"].some((term) =>
      value.includes(term),
    )
  )
    return "danger";
  if (
    ["pending", "review", "medium", "follow", "due", "progress"].some((term) =>
      value.includes(term),
    )
  )
    return "warning";
  if (
    ["applied", "interview", "planned", "scheduled", "priority"].some((term) =>
      value.includes(term),
    )
  )
    return "info";
...
export function StatusBadge({
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] font-medium capitalize leading-none",
        tones[resolvedTone],
        className,
      )}
    >
